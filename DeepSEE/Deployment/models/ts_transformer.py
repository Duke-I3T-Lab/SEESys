    ...
    if (task == "classification") or (task == "regression"):
        ...
        if config['model'] == 'LINEAR':
            ...
            return TSTransformerEncoderClassiregressor(feat_dim, max_seq_len, config['d_model'],
                                                        ...
                                                        activation=config['activation'],
                                                        norm=config['normalization_layer'], freeze=config['freeze'])
        elif config['model'] == 'DWTtransformer':
            return TSDWTTransformerEncoderClassiregressor(feat_dim, max_seq_len, config['d_model'],
                                                        config['num_heads'],
                                                        config['num_layers'], config['dim_feedforward'],
                                                        num_classes=num_labels,
                                                        dropout=config['dropout'], pos_encoding=config['pos_encoding'],
                                                        activation=config['activation'],
                                                        norm=config['normalization_layer'], freeze=config['freeze'])
        elif config['model'] == 'PatchTSMixerRegression':
            return PatchTSMixerRegression(feat_dim, max_seq_len, config['d_model'],
                                                        config['num_heads'],
                                                        config['num_layers'], config['dim_feedforward'],
                                                        num_classes=num_labels,
                                                        dropout=config['dropout'], pos_encoding=config['pos_encoding'],
...
class TSDWTTransformerEncoderClassiregressor(nn.Module):
    """
    Simplest classifier/regressor. Can be either regressor or classifier because the output does not include
    softmax. Concatenates final layer embeddings and uses 0s to ignore padding embeddings in final output layer.
    """
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
                 dropout=0.1, pos_encoding='fixed', activation='gelu', norm='BatchNorm', freeze=False):
        super(TSDWTTransformerEncoderClassiregressor, self).__init__()
        ...
        # !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
        feat_dim = feat_dim * 3 # since we have level=2 in DWT
        self.project_inp = nn.Linear(feat_dim, d_model)
        self.pos_enc = get_pos_encoder(pos_encoding)(d_model, dropout=dropout*(1.0 - freeze), max_len=max_len)
...
class PatchTSMixerRegression(nn.Module):
    """
    Simplest classifier/regressor. Can be either regressor or classifier because the output does not include
    softmax. Concatenates final layer embeddings and uses 0s to ignore padding embeddings in final output layer.
    """
    ...
    def __init__(self, feat_dim, max_len, d_model, n_heads, num_layers, dim_feedforward, num_classes,
        ...
        config = PatchTSMixerConfig(context_length = max_len, 
        ...
        self.model = PatchTSMixerForRegression(config)
    ...
    def forward(self, X, padding_masks):
        ...
        Returns:
            output: (batch_size, num_classes)
        """
        return self.model.forward(past_values=X).prediction_outputs